...
`main.jsp`のJavaScriptが5秒ごとに最新20件を取得します。
「さらに読み込む」を押すと、最後に表示したIDをカーソルとして次の20件を取得します。

```text
main.jspのJavaScript
  └─ GET /MutterList?limit=20
       └─ MutterList.java
            └─ GetMutterListLogic
                 └─ MutterDAO.findLatest(21)
                      └─ MutterPageを返す
                           └─ MutterListResponseへ変換
                                └─ JacksonでJSONを出力

「さらに読み込む」
  └─ GET /MutterList?cursor=最後のID&limit=20
       └─ MutterDAO.findByCursor(cursor, 21)
```
...
旧画面用の`/Main`、`/UpdateMutter`、`/DeleteMutter`、`/MutterList`も引き続き利用できます。
